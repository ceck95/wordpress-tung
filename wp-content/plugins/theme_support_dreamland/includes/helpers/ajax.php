...
class Bunch_Ajax
{
	...
	{
		$method = dreamland_set( $_REQUEST, 'subaction' );
	...
	function tweets()
	{
		...
		$cb = new Codebird;
		$method = dreamland_set( $_POST, 'method' );
		
		$theme_options = _WSH()->option();
		//printr($theme_options);
		$api = dreamland_set($theme_options, 'twitter_api');
		$api_secret = dreamland_set($theme_options, 'twitter_api_secret');
		$token = dreamland_set($theme_options, 'twitter_token');
		$token_secret = dreamland_set($theme_options, 'twitter_token_Secret');
		if( !$api && $api_secret ) 
		{ 
		...
		$reply = (array) $cb->statuses_userTimeline(array('count'=>dreamland_set( $_POST, 'number' ), 'screen_name'=>dreamland_set($_POST, 'screen_name')));
		if( isset( $reply['httpstatus'] ) ) unset( $reply['httpstatus'] );
		foreach( $reply as $k => $v ){
			
			//if( $k == 'httpstatus' ) continue;
			$time = human_time_diff( strtotime( dreamland_set( $v, 'created_at') ), current_time('timestamp') ) . __(' ago', BUNCH_NAME);
			$text = preg_replace('@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?)?)@', '<a href="$1" target="_blank">$1</a>', dreamland_set( $v, 'text'));
	...
	function contact_form_submit()
	{
		...
		$t->validation->set_rules('contact_subject','<strong>'.__('Subject', BUNCH_NAME).'</strong>');
		
		//$t->validation->set_rules('contact_priority','<strong>'.__('Priority', BUNCH_NAME).'</strong>');
		
		$t->validation->set_rules('contact_message','<strong>'.__('Message', BUNCH_NAME).'</strong>', 'required|min_length[5]');
		if( dreamland_set($settings, 'contact_captcha_status'))
		{
			include_once( get_template_directory().'/includes/thirdparty/recaptchalib.php');
			$privatekey = dreamland_set($settings, 'recaptcha_private');
		...
		if($t->validation->run() !== FALSE && empty($t->validation->_error_array))
		{
			...
			//$message .= __("Patient Type:\t", BUNCH_NAME).$ptype."\r\n";
			//$message .= __("Priority:\t", BUNCH_NAME).$priority."\r\n";
			//$message .= "\r\n".__("Contact Website:\t", BUNCH_NAME).$t->validation->post('contact_website')."\r\n";
			//$message .= "\r\n".__("Contact Subject:\t", BUNCH_NAME).dreamland_set( $_POST, 'contact_subject')."\r\n";
	...
	function wishlist()
	{
		...
		if( is_user_logged_in() ){
			...
			$data_id = dreamland_set( $_POST, 'data_id' );
			if( $meta && is_array( $meta ) ){
				...
				$newmeta = array_merge( array( dreamland_set( $_POST, 'data_id' ) ), $meta );
	...
	function wishlist_del()
	{
		...
		if( is_user_logged_in() ){
			...
			if( $meta && is_array( $meta ) ){
				...
				update_user_meta( $current_user->ID, '_ja_product_wishlist', array( dreamland_set( $_POST, 'data_id' ) ) );
	...
	function likeit()
	{
		...
		$post_id = dreamland_set( $_POST, 'data_id' );
