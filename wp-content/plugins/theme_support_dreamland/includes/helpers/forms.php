...

{
...

	{
		...
		$this->_old_hash = dreamland_set( $_COOKIE, 'bunch_form_builder_hash' );
...

	{
		...
		$this->fields = dreamland_set( $fields, 'field' );
		...
		echo form_open( dreamland_set( $this->meta, 'form_action' ), 'method="'.dreamland_set( $this->meta, 'form_method', 'post' ).'"' );
...

		{
			...
			$this->type = $this->type( dreamland_set( $field, 'type' ) );
...

	{
		...
		$default = dreamland_set( $settings, $name ) ? dreamland_set( $settings, $name ) : $default;
...

		{
			...
			case "textarea":
				...
				$settingsvalue = empty($user_settings[$name]) ? dreamland_set($settings, 'value') : $user_settings[$name];
			...
			case "switch" : 
				...
				$checked = (dreamland_set($user_settings, $field) == 'on') ? 'checked="checked"' : '';
			...
			case 'file':
				...
				if(dreamland_set($user_settings, $field)) $html['preview'] = dreamland_set($user_settings, $field);
			...
			case "hidden":
				...
				$html['element'] = form_input(array_merge(array('type'=>'hidden','name'=>$field,'value'=>$default_value,'id'=>$field), dreamland_set($settings, 'attrs')));
...

	{
		...
		$loop = dreamland_set( $field, $type.'_value');
		...
		if( in_array( $type, $this->loops ) ){
			...
			$label = dreamland_set( $this->labels, $type );
			...
			foreach( $loop as $l ){
				...
				$new['options'][dreamland_set($l, $label.'_value')] = dreamland_set($l, $label.'_label');
...

	{
		...
		return dreamland_set( $array, $type, $type );
...

	{
...

		{
			...
			$field_name = dreamland_set( $f, 'name' );
			...
			$placehold = dreamland_set( $f, 'placeholder' );


			


			foreach( dreamland_set( $f, 'validation' ) as $valid ) $validation[] = $valid;
			...
			$t->validation->set_rules( $field_name ,'<strong>'.dreamland_set( $f, 'placeholder' ).'</strong>', implode( '|', $validation ) );
		...
		if( dreamland_set( $_POST, 'form_secret' ) !== $this->_old_hash ) 
...

		{
			...
			$contact_to = (dreamland_set( $this->meta, 'form_email')) ? dreamland_set( $this->meta, 'form_email') : get_option('admin_email');
			...
			$response = dreamland_set($this->meta, 'form_success_msg') ? $this->meta['form_success_msg'] : sprintf( __('Form is successfully submitted, we\'ll response you shortly.',BUNCH_NAME), $name);
