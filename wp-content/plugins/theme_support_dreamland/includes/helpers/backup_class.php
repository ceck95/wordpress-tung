...
class Bunch_Backup_class
{
	...
	function revslider_export( $file = '' )
	{
		...
		foreach( $sliders as $k=>$s )
		{
			...
			foreach( $slides as $ss )
			{
				...
				if( $slider_id == dreamland_set( $ss, 'slider_id' ) )
	...
	function theme_options_import($file = '')
	{
		...
		$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."terms WHERE ".$wpdb->prefix."terms.slug = 'main-menu' OR ".$wpdb->prefix."terms.slug = 'onepage-menu'");
		if($res){
			$nav_menu = array('');
			$nav_menu['nav_menu_locations']['main_menu'] = $res[0]->term_id;
			$nav_menu['nav_menu_locations']['onepage_menu'] = $res[1]->term_id;
