...

{
...

	{
		...
		$post_type = dreamland_set( $post, 'post_type');


		$settings = get_post_meta( dreamland_set($post, 'ID' ), '_bistro_'.$post_type.'_settings', true);


		


		//$fields = $t->_fields_enqueue(dreamland_set($this->_fields, $post_type), $settings); 


		//printr($fields);


		$fields = dreamland_set( $this->_fields, $post_type );
		...
		if( $fields && is_array( $fields ) ): ?>
			...
			<?php foreach( $fields as $f):
                ...
                <div class="fields_set" >
                	...
                	<label><strong><?php echo dreamland_set( $f, 'title'); ?></strong></label>


                    <div class="field">


                    	<?php echo $nph->_field_input($f, dreamland_set($settings, dreamland_set( $f, 'id' )) ); 	?>
...

	{
		...
		$post_type = dreamland_set( $_POST, 'post_type' );


		$setting = dreamland_set($this->_fields, $post_type);
		...
		$data = dreamland_set( $_POST, $post_type);//array_intersect_key( $_POST, $setting);
		...
		if( $post_type == 'bistro_deal') update_post_meta( $post_id, '_bistro_bistro_deal_date', strtotime(dreamland_set( $data, 'end_date')));
