...
add_action( 'init', 'dreamland_theme_init');
...
if( !function_exists('dreamland_printr' ) ) {

	function dreamland_printr($data)
...
if( !function_exists('dreamland_font_awesome' ) ) {

	function dreamland_font_awesome( $index )

	{

		$array = array_values($GLOBALS['dreamland_font_awesome']);

		if( $font = dreamland_set($array, $index )) return $font;
...
if( !function_exists('dreamland_load_class' ) ) {

	function dreamland_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
...
dreamland_load_class( 'enqueue', 'helpers', false );

dreamland_load_class( 'seo', 'helpers', false );

dreamland_load_class( 'bootstrap_walker', 'helpers', false );
...
if( dreamland_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...

{
	...
	if( is_admin() ) {
		...
		if( dreamland_set( $_GET, 'page' ) == 'dreamland'.'_option' ) 

		{

			if( dreamland_set( $_GET, 'bunch_dummydata_export' ) ){
			...
			/*if( dreamland_set( $_GET, 'bunch_dummydata_import' ) ){ 
	...
	if( function_exists( 'dreamland_vc_map' )) 
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'dreamland'.'_theme_options',                      // options key in db, required

		'page_slug'             => 'dreamland'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'dreamland' ), // page title

		'menu_label'            => esc_html__( 'Theme Options', 'dreamland' ), // menu label

	));

	

		_WSH()->user_extra( array('facebook'=>esc_html__('Facebook', 'dreamland'), 'twitter'=>esc_html__('Twitter', 'dreamland'), 'dribbble'=>esc_html__('Dribble', 'dreamland'), 'github'=>esc_html__('Github', 'dreamland'),

	'flickr'=>esc_html__('Flickr', 'dreamland'), 'google-plus'=>esc_html__('Google+', 'dreamland'), 'youtube'=>esc_html__('Youtube', 'dreamland')) );
...
//include_once get_template_directory() . '/vp_new/loader.php';
...
// shortocode generators
 ...
 $tmpl_sg = array(
  ...
  'modal_title' => esc_html__( 'Pretty Shortcodes', 'dreamland'),

  'button_title' => esc_html__('Pretty', 'dreamland'),
