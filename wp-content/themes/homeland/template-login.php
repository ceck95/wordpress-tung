		...
		<div class="inside clearfix">
			...
			<?php
				...
				if ( ! is_user_logged_in() ) :
					if(!empty($homeland_ptitle)) : echo '<h2><span>' . esc_html( $homeland_ptitle ) . '</span></h2>';
					else : the_title('<h2><span>', '</span></h2>'); endif; 

					if(!empty($homeland_subtitle)) : echo '<h3>' . stripslashes ( $homeland_subtitle ) . '</h3>';
					else : ?>
						<h3><?php esc_html_e( 'Already a member? Please login here', 'homeland' ); ?></h3><?php
					endif;
					...
					?>
						<div class="login-links">
							<a href="<?php echo wp_registration_url(); ?>"><?php esc_html_e( 'Register', 'homeland' ); ?></a> | <a href="<?php echo wp_lostpassword_url(); ?>" title="Lost Password"><?php esc_html_e( 'Lost your Password?', 'homeland' ); ?></a>
						</div>
					<?php
				else :
