<?php 
	...
	$homeland_agent_property_order = esc_attr( get_option('homeland_agent_property_order') );
	$homeland_agent_property_orderby = esc_attr( get_option('homeland_agent_property_orderby') );
	$homeland_agent_property_limit = esc_attr( get_option('homeland_agent_property_limit') );
	$homeland_agent_hide_property = esc_attr( get_option('homeland_agent_hide_property') );
	$homeland_page_nav = esc_attr( get_option('homeland_pnav') );

	$homeland_agent_list_header_label = !empty( $homeland_agent_list_header ) ? $homeland_agent_list_header : esc_html__( 'Listed Properties', 'homeland' );
	...
	<section class="theme-pages">
		<div class="inside clearfix">
			...
			<div class="<?php echo esc_attr( $homeland_agent_profile_class ); ?>">
				<div class="agent-container">
		    		...
		    		<div class="agent-image">
		    			<?php if(!empty($homeland_custom_avatar)) : ?>
    						<img src="<?php echo esc_url( $homeland_custom_avatar ); ?>" alt="<?php esc_html_e('Agent Image', 'homeland'); ?>">
    					<?php	else : 
		    				...
		    				<ul class="clearfix">
		    					<?php
		    						...
		    						if(!empty($homeland_agent_email)) : 
		    							echo '<li class="last">
		    									<a href="mailto:'. esc_html( $homeland_agent_email ) .'" target="_blank">
		    		...
		    		<div class="agent-desc">
		    			<h4><?php echo esc_html( $homeland_agent_name ); ?></h4>
		    			<?php echo do_shortcode( wpautop( $homeland_agent_desc ) ); ?>
		    			<label class="more-info">
		    				<?php 
		    					if(!empty($homeland_agent_telno)) : ?>
		    						<span><i class="fa fa-phone"></i><strong><?php esc_html_e( 'Tel no', 'homeland' ); echo ":"; ?></strong> <?php echo esc_html( $homeland_agent_telno ); ?></span><?php
		    					endif;
		    					if(!empty($homeland_agent_mobile)) : ?>
		    						<span><i class="fa fa-mobile"></i><strong><?php esc_html_e( 'Mobile', 'homeland' ); echo ":"; ?></strong> <?php echo esc_html( $homeland_agent_mobile ); ?></span><?php
		    					endif;
		    					if(!empty($homeland_agent_fax)) : ?>
		    						<span><i class="fa fa-print"></i><strong><?php esc_html_e( 'Fax', 'homeland' ); echo ":"; ?></strong> <?php echo esc_html( $homeland_agent_fax ); ?></span><?php
		    					endif;
			    				?>
		    			</label>
		    		</div>
	            ...
	            <h3>
	            	<?php 
	            		...
	            		echo esc_html( $homeland_agent_list_header_label ); 
	            	?>
	            </h3>
	            ...
	            <?php 
								$args = array( 
									...
									'author' => $curauth->ID, 
									'orderby' => $homeland_agent_property_orderby, 
									'order' => $homeland_agent_property_order, 
									'posts_per_page' => $homeland_agent_property_limit,
									'paged' => $paged 
								);
								$wp_query = new WP_Query( $args );  

	              if ( $wp_query->have_posts() ) : ?>
	                <div class="<?php echo esc_attr( $homeland_agent_masonry_class ); ?>">
	                  <ul class="clearfix">
	                    <?php
	                    	if($homeland_agent_profile_layout == "Fullwidth") :
			...
			<?php if($homeland_agent_profile_layout != "Fullwidth") : ?>
				<div class="<?php echo esc_attr( $homeland_agent_sidebar_class ); ?>"><?php get_sidebar(); ?></div>
			<?php endif; ?>
		</div>
	</section>
