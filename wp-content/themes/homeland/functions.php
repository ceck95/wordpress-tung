<?php
	...
	*/
	...
	get_template_part( 'includes/lib/custom', 'fields' );
	get_template_part( 'includes/lib/custom', 'widgets' );
	get_template_part( 'includes/lib/custom', 'functions' );
	get_template_part( 'includes/lib/custom', 'css' );
	get_template_part( 'includes/lib/tgm/activation', '' );
	get_template_part( 'includes/customizer/option', 'customizer' );
	...
	if ( ! function_exists( 'homeland_script_styles_reg' ) ) :
		function homeland_script_styles_reg () {
			...
			wp_enqueue_style( 'font-awesome', get_theme_file_uri( '/includes/font-awesome/css/font-awesome.min.css' ), array(), '4.7.0', 'all' );
			
			// jQuery Scripts
			wp_enqueue_script( 'masonry' );
			wp_enqueue_script( 'jquery-superfish', get_theme_file_uri( '/js/superfish.min.js' ), array( 'jquery' ), '', true );
			wp_enqueue_script( 'jquery-retina', get_theme_file_uri( '/js/retina.min.js' ), array( 'jquery' ), '', true );
			wp_enqueue_script( 'jquery-hover-modernizr', get_theme_file_uri( '/js/modernizr.custom.js' ), array( 'jquery' ), '', true );
			wp_enqueue_script( 'jquery-elastic', get_theme_file_uri( '/js/jquery.elastislide.min.js' ), array( 'jquery' ), '', true );

			// Flexslider
			wp_enqueue_style( 'jquery-flexslider', get_theme_file_uri( '/js/flexslider/flexslider.css' ), array(), '', 'all' );
			wp_enqueue_script( 'jquery-flexslider', get_theme_file_uri( '/js/flexslider/jquery.flexslider-min.js' ), array( 'jquery' ), '', true );

			// Magnefic Popup
			wp_enqueue_style( 'jquery-magnific', get_theme_file_uri( '/js/magnific-popup/magnific-popup.css' ),	array(), '', 'all' );
			wp_enqueue_script( 'jquery-magnific', get_theme_file_uri( '/js/magnific-popup/jquery.magnific-popup.min.js' ), array( 'jquery' ), '', true );

			// Select2 
			wp_enqueue_style( 'jquery-select2', get_theme_file_uri( '/js/select2/select2.css' ), array(), '', 'all' );
			wp_enqueue_script( 'jquery-select2', get_theme_file_uri( '/js/select2/select2.min.js' ), array( 'jquery' ), '', true );
			...
			wp_register_script( 'google-map-markerclusterer', get_theme_file_uri( '/js/markerclusterer.min.js' ) );
			wp_register_script( 'google-map', get_theme_file_uri( '/js/gmaps.min.js' ) );
			...
			if($homeland_bg_type == "Image" && ($homeland_theme_layout == "Boxed" || $homeland_theme_layout == "Boxed Left")) :
				wp_enqueue_script( 'jquery-backstretch', get_theme_file_uri( '/js/jquery.backstretch.min.js' ), array( 'jquery' ), '', true );
			endif;
			...
			if(is_singular('homeland_properties')) : 
				wp_enqueue_script( 'google-map-api' );
				wp_enqueue_script( 'google-map' );
				wp_enqueue_script( 'jquery-validation', get_theme_file_uri( '/js/jquery.validate.min.js' ), array( 'jquery' ), '', true );
			...
			if(is_page_template('template-blog.php') || is_page_template('template-blog-3cols.php') || is_page_template('template-blog-4cols.php') || is_page_template('template-blog-fullwidth.php') || is_page_template('template-blog-grid-left-sidebar.php') || is_page_template('template-blog-grid.php') || is_page_template('template-blog-left-sidebar.php') || is_page_template('template-blog-timeline.php') || is_page_template('template-blog-2cols.php') || is_page_template('template-blog-list-alternate.php') || is_single() || is_archive()) :
				wp_enqueue_style( 'jquery-video', get_theme_file_uri( '/js/video/video-js.css' ), array(), '', 'all' );
				wp_enqueue_script( 'jquery-video', get_theme_file_uri( '/js/video/video.js' ), array( 'jquery' ), '', true );
				wp_enqueue_script( 'jquery-tipsy', get_theme_file_uri( '/js/tipsy/jquery.tipsy.min.js' ), array( 'jquery' ), '', true );
			endif;
			...
			if(is_page_template('template-coming-soon.php')) :
				wp_enqueue_script( 'jquery-countdown-plugin', get_theme_file_uri( '/js/countdown/jquery.plugin.min.js' ), array( 'jquery' ), '', true );
				wp_enqueue_script( 'jquery-countdown', get_theme_file_uri( '/js/countdown/jquery.countdown.min.js' ), array( 'jquery' ), '', true );
			endif;

			// RTL Enable
			if ( is_rtl() ) :
				wp_enqueue_style( 'style-rtl', get_theme_file_uri( '/css/rtl-main.css' ) );	
			endif;
			...
			if(empty($homeland_site_layout)) :
				...
				if ( is_rtl() ) :
					wp_enqueue_style( 'style-rtl-responsive', get_theme_file_uri( '/css/rtl-responsive.css' ) );	
				endif;
			
				wp_enqueue_script( 'jquery-mobile-menu', get_theme_file_uri( '/js/jquery.mobilemenu.min.js' ), array( 'jquery' ), '', true );
			endif;

			// Custom jQueries
			wp_enqueue_script( 'homeland-custom-js', get_theme_file_uri( '/js/custom.js' ), array( 'jquery' ), '', true );
		}
	endif;
				    	...
				    	<?php if(is_tax( 'homeland_property_location' ) || is_tax( 'homeland_property_type' ) || is_tax( 'homeland_property_status' ) || is_tax( 'homeland_property_amenities' )) : ?>
			    			map = new GMaps({
						      ...
						      lat: <?php echo esc_html( $homeland_lat_main_value ); ?>,
									...
									styles : <?php echo wp_kses_post( $homeland_map_styles ); ?>,
									markerClusterer: function(map) {
								      ...
								      styles: [{
												...
												<?php else : ?>
													url: "<?php echo get_template_directory_uri() ?>/img/clusterer.png"
												<?php endif; ?>
											}] 
								   	});
									}
	...
	if ( ! function_exists( 'homeland_google_map_property' ) ) :
		function homeland_google_map_property() {
			...
			$homeland_property_map_zoom_value = !empty($homeland_property_map_zoom) ? $homeland_property_map_zoom : '8';
			...
			<script type="text/javascript">
				(function($) {
		      	...
		      	map.addMarker({
			        ...
			        <?php if(!empty($homeland_home_map_icon)) : ?>icon: image, <?php endif; ?>
			        infoWindow: {
				    		content: '<p><?php echo wp_kses_post( $homeland_contact_address ); ?></p>'
				    	}
		      	});
			   	});
				})(jQuery);					
	...
	if ( ! function_exists( 'homeland_login_image' ) ) :
		function homeland_login_image() {
			...
			wp_enqueue_style( 'homeland-custom-style', get_theme_file_uri( '/includes/admin/admin-style.css' ) );
	...
	if ( ! function_exists( 'homeland_get_page_title' ) ) :
		function homeland_get_page_title() {
			...
			if (is_post_type_archive('homeland_properties') || is_post_type_archive('homeland_portfolio') || is_post_type_archive('homeland_services')) : 
				the_archive_title('<h2 class="ptitle">', '</h2>');
	...
	if ( ! function_exists( 'homeland_get_page_title_subtitle_desc' ) ) :
		function homeland_get_page_title_subtitle_desc() {
			...
			foreach($homeland_theme_pages as $homeland_page) :
				...
				$homeland_page_subtitle = esc_attr( get_post_meta( $homeland_page->ID, "homeland_subtitle", true ) );
				$homeland_post_title = esc_attr( $homeland_page->post_title );
			...
			if(!empty($homeland_page_subtitle)) : 
				echo '<h4 class="subtitle"><label>' . stripslashes ( $homeland_page_subtitle ) . '</label></h4>';
			endif;
		}
	endif;
	...
	if ( ! function_exists( 'homeland_get_property_category' ) ) :
		function homeland_get_property_category() {
			global $homeland_properties_page_url, $homeland_properties_page_2cols_url, $homeland_properties_page_3cols_url, $homeland_properties_page_4cols_url; 
				...
				if(is_page_template('template-properties.php') || is_page_template('template-properties-2cols.php') || is_page_template('template-properties-3cols.php') || is_page_template('template-properties-4cols.php')) : $homeland_current_class = "current-cat"; 
				endif;
			?>
	...
	if(!function_exists('homeland_advance_property_search')) :	
		function homeland_advance_property_search($homeland_search_args) {
			...
			if((!empty($_GET['city']))) : 
				$homeland_tax_query[] = array(
					'taxonomy' => 'homeland_property_location', 'field' => 'slug', 'terms' => $_GET['city']
				); 
			endif;
      ...
      if(isset($_GET['min-price'])) :
      	...
      	if( $homeland_min_price > 0 ) : 
      		$homeland_meta_query[] = array( 
      			'key' => 'homeland_price', 'value' => $homeland_min_price, 'type' => 'NUMERIC', 'compare' => '>=' 
      		); 
      	endif;
      endif;
      ...
      if(isset($_GET['max-price'])) :
        ...
        if( $homeland_max_price > 0 ) : 
        	$homeland_meta_query[] = array( 
        		'key' => 'homeland_price', 'value' => $homeland_max_price, 'type' => 'NUMERIC', 'compare' => '<=' 
        	); 
        endif;
	 		endif;
  ...
  add_filter('homeland_advance_search_parameters', 'homeland_advance_property_search');
				...
				<div class="filter-sort-order">
					<form action="<?php echo esc_url( $homeland_path ); ?>" method="get" class="form-sorting-order">
						<?php 
							if(is_page_template('template-property-search.php')) : 
								if(isset($_GET['pid']) != "") :
									?><input type="hidden" name="pid" value="<?php echo $_GET['pid']; ?>"><?php
								endif;
								if(isset($_GET['city']) != "") :
									?><input type="hidden" name="city" value="<?php echo $_GET['city']; ?>"><?php
								endif;
								if(isset($_GET['status']) != "") :
									?><input type="hidden" name="status" value="<?php echo $_GET['status']; ?>"><?php
								endif;
								if(isset($_GET['type']) != "") :
									?><input type="hidden" name="type" value="<?php echo $_GET['type']; ?>"><?php
								endif;
								if(isset($_GET['bed']) != "") :
									?><input type="hidden" name="bed" value="<?php echo $_GET['bed']; ?>"><?php
								endif;
								if(isset($_GET['bath']) != "") :
									?><input type="hidden" name="bath" value="<?php echo $_GET['bath']; ?>"><?php
								endif;
								if(isset($_GET['min-price']) != "") :
									?><input type="hidden" name="min-price" value="<?php echo $_GET['min-price']; ?>"><?php
								endif;
								if(isset($_GET['max-price']) != "") :
						...
						?>
						<label for="input_order"><?php esc_html_e('Order', 'homeland'); ?></label>
					 	<select name="order" id="input_order">
				      <?php
								$homeland_filter_order = @$_GET['order'];

								if($homeland_filter_ordering == "ASC") :
								...
								foreach($homeland_array as $homeland_order_option_value => $homeland_order_option) : 
									...
									?>
		              <option value="<?php echo $homeland_order_option_value; ?>" <?php echo $homeland_filter_property_order; ?>><?php echo esc_html( $homeland_order_option ); ?></option><?php
		            endforeach;
							?>		
				    </select>
				    <label for="input_sort"><?php esc_html_e('Sort By', 'homeland'); ?></label>
					 	<select name="sortby" id="input_sort">
							<?php
								$homeland_filter_sort = @$_GET['sortby'];
		              ...
		              <option value="<?php echo $homeland_sort_option_value; ?>" <?php if($homeland_filter_sort == $homeland_sort_option_value) : echo "selected='selected'"; endif; ?>><?php echo esc_html( $homeland_sort_option ); ?></option><?php
		            endforeach;
							?>		
						</select>                                                                                   
				   </form>	
	...
	if(!function_exists('homeland_default_img_bg')) :	
		function homeland_default_img_bg() {
			...
			$homeland_empty_bg = "https://goo.gl/FVAWLr";
	...
	if(!function_exists('homeland_header_image')) :	
		function homeland_header_image() {
			...
			// Title
			$homeland_title_block = !empty($homeland_page_hd_image) ? 'page-title-block' : 'page-title-block-default';
	   	...
	   	if(isset($_GET['order'])) :
			...
			if(isset($_GET['sortby'])) :
				...
				// Date
					...
					if(isset($_GET['sortby']) == "price") :
					 	$homeland_property_list_args['meta_query'] = $homeland_search_query;
						$homeland_property_list_args['orderby'] = 'meta_value_num';
					elseif(isset($_GET['sortby']) == "title") :
					  $homeland_property_list_args['orderby'] = 'title';
					elseif(isset($_GET['sortby']) == "rand") :
					  $homeland_property_list_args['orderby'] = 'rand';
					else :
						$homeland_property_list_args['orderby'] = 'date';
					endif;
