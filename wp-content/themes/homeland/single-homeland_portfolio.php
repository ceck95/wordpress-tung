<?php 
	...
	$homeland_hide_portfolio_prevnext = esc_attr( get_option('homeland_hide_portfolio_prevnext') );
	...
	$homeland_portfolio_slider_class = !empty($homeland_portfolio_static_image) ? 'portfolio-static-images' : 'portfolio-flexslider';
	...
	<section class="theme-pages">
		<div class="inside clearfix">
				...
				<?php
						...
						if ( post_password_required() ) :
							...
							while (have_posts()) : the_post(); ?>
								<article id="post-<?php the_ID(); ?>" <?php sanitize_html_class( post_class() ); ?>>
									...
									<div class="portfolio-list-desc clearfix">
										<?php the_title( '<h4>', '</h4>' ); ?>
										...
										<?php 
											...
											if(!empty($homeland_website)) : 
												?>
													<a href="<?php echo esc_url( $homeland_website ); ?>" class="live-demo" target="_blank">
														<?php esc_html_e( 'Live Demo', 'homeland' ); ?>
													</a>
												<?php
											endif;
					...
					if(empty($homeland_hide_portfolio_prevnext)) : ?>
						<div class="post-link-blog clearfix">
							<span class="prev">
								<?php 
									previous_post_link( '%link', '&larr;&nbsp;' . esc_html__( 'Previous Portfolio', 'homeland' ), '' ); 
								?>
							</span>
							<span class="next">
								<?php 
									next_post_link( '%link', esc_html__( 'Next Portfolio', 'homeland' ) . '&nbsp;&rarr;', '' ); 
								?>
							</span>
						</div><?php
					endif;
			...
			<?php if($homeland_single_portfolio_layout == "Right Sidebar" || $homeland_single_portfolio_layout == "Left Sidebar") : ?>
				<div class="<?php echo esc_attr( $homeland_portfolio_class_sidebar ); ?>"><?php get_sidebar(); ?></div>
			<?php endif; ?>
		</div>
	</section>
